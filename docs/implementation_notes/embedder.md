...
1. **Now:** DINOv3 backbone, CLS readout only, `cuttle embed` wired into the CLI, writing the single-combined-array duck-typed output described in "CLI: `cuttle embed`" below. Pause here and commit once this works end to end.
2. **Next:** update `cuttle_patterns.latents.load_latents` (renamed from `embeddings.py` on 2026-09-20 — see "Module rename" below) to add a second branch reading the new `embeddings.npy` + `manifest.parquet` layout (alongside its existing per-frame-`.npy` BEAST layout, unchanged for real `beast train` output) — this is the only change `cuttle reduce`/`cuttle cluster` need, since both already go through `load_latents`/`split_latent_spaces` and never touch the filesystem layout directly themselves.
3. **After that:** add the `meanpatch_uniform`/`meanpatch_taper`/`gram_*` readouts from sections 2/3 below, each reachable via its own `cuttle embed --readout ...` invocation (see the "no cross-readout backbone sharing" note in section 1).
