...
from cuttle_patterns.embedders.readouts import CLS_READOUT_NAME, READOUTS_BY_NAME
from cuttle_patterns.embedders.vgg import BACKBONE_NAME as VGG_BACKBONE_NAME
from cuttle_patterns.embedders.vgg import VGGBackbone
...
def build_embedder(
    ...
    readout_name: str,
    device: torch.device,
    vgg_layer: str | None = None,
    readout_kwargs: dict | None = None,
) -> Embedder:
    """Build the one embedder a `cuttle embed` run uses.

    Validates `readout_name` (and the VGG/CLS incompatibility below) before loading any
    weights, so an invalid combination fails fast rather than after a slow
    (network-dependent) backbone load.
    ...
    Args:
        ...
        resolution: square input side length, in pixels; must be a multiple of 16 for a
            DINOv3 backbone, or of `vgg_layer`'s downsampling stride for `vgg19`.
        readout_name: one of `cuttle_patterns.embedders.readouts.READOUTS_BY_NAME`'s
            keys.
        device: device to load the backbone onto.
        vgg_layer: one of `cuttle_patterns.embedders.vgg.LAYER_TO_INDEX`'s keys;
    ...
    Raises:
        ValueError: if `backbone_arch`, `readout_name`, or `vgg_layer` is unrecognized;
            if `resolution` doesn't divide evenly for the chosen backbone; or if
            `readout_name == 'cls'` with `backbone_arch == 'vgg19'`, which has no
            CLS-token equivalent.
    """
    ...
    if backbone_arch == VGG_BACKBONE_NAME:
        ...
        backbone = VGGBackbone(vgg_layer or DEFAULT_VGG_LAYER, resolution, device)
